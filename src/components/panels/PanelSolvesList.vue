    ...
    <panel panelTitle="Session Solves">
        ...
        <table class="table table-striped" v-else>
            ...
            <tr v-for="solve in solves">
                ...
                <td class="time-column" :class="{ 'penalty-active': solve.penalty !== SolvePenalty.NONE }">{{ solve.formattedTime }}</td>
                <td class="scramble-column visible-md visible-lg">{{ solve.scramble }}</td>
                <td class="actions-column">
                    <span class="penalty-link" :class="{ 'penalty-active': solve.penalty === SolvePenalty.PLUSTWO }" @click="setPenalty(solve, SolvePenalty.PLUSTWO)">+2</span>
                    <span class="penalty-link" :class="{ 'penalty-active': solve.penalty === SolvePenalty.DNF }" @click="setPenalty(solve, SolvePenalty.DNF)">DNF</span>
